...
import { faRetweet, faHeart as faHaertFilled } from '@fortawesome/free-solid-svg-icons';
      ...
      <div className='flex-1'>
        <div className='flex flex-wrap items-center gap-x-3'>
          ...
          <div className='font-bold text-lg'>
            <Link href={`/${tweet.user.slug}`}>{tweet.user.name}
            </Link>
          </div>

          <div className='text-xs text-gray-500'>@{tweet.user.slug} - {formatRelativeTime(tweet.dataPost)}
          </div>
        </div>

        <div className='py-4 text-lg'>{tweet.body}
        </div>
